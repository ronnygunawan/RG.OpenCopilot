...
The executor service supports **Windows and Linux host systems**. Code execution always happens in **Linux-based Docker containers** (`mcr.microsoft.com/dotnet/sdk:10.0` which is Ubuntu-based), ensuring consistent behavior regardless of the host operating system.
...
**Important:** Container paths are always Linux-style paths (forward slashes), while host paths follow the host OS conventions.
